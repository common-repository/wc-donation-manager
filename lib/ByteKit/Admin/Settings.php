...
namespace WooCommerceDonationManager\ByteKit\Admin;

/**
 * Settings class
 *
 * @since 1.0.0
 * @package WooCommerceDonationManager\ByteKit
 */
abstract class Settings {
	...
	public function save_settings() {
		...
		if ( $this->save_fields( $settings ) ) {
			add_settings_error( $class_name, 'response', __( 'Settings saved.', 'wc-donation-manager' ), 'updated' );
			return true;
		}
	...
	public function register_scripts() {
		...
		wp_register_script( $basename, plugins_url( 'assets/js/settings.js', dirname( __FILE__ ) ), array( 'jquery' ), $version, true );
	}
			...
			foreach ( $promo_plugins as $promo_plugin ) {
				$promo_plugin = wp_parse_args(
					...
					array(
						...
						'basename'    => '',
						'slug'        => '',
						'badge'       => esc_html__( 'Recommended', 'wc-donation-manager' ),
						'button'      => esc_html__( 'Install Now', 'wc-donation-manager' ),
						'installed'   => false,
					)
				);
	...
	protected function output_support_widget() {
		...
		if ( ! empty( $support_links ) ) {
			...
			<div class="bk-card">
				<div class="bk-card__header">
					<?php esc_html_e( 'Need Help?', 'wc-donation-manager' ); ?>
				</div>
				<div class="bk-card__body">
					<ul>
	...
	public function get_support_links() {
		return array(
			'facebook'        => array(
				'label' => __( 'Join our Community', 'wc-donation-manager' ),
				'url'   => 'https://www.facebook.com/groups/pluginever',
			),
			'feature-request' => array(
				'label' => __( 'Request a Feature', 'wc-donation-manager' ),
				'url'   => 'https://www.pluginever.com/contact/',
			),
			'bug-report'      => array(
				'label' => __( 'Report a Bug', 'wc-donation-manager' ),
				'url'   => 'https://www.pluginever.com/contact/',
			),
		);
	}
