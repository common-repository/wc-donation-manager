...
use WooCommerceDonationManager\ByteKit\Plugin;
...
class Notices {
		...
		foreach ( $this->notices as $notice ) {
			if ( $this->should_display( $notice ) ) {
				...
				printf(
					...
					$notice['dismissible'] ? '<button type="button" class="notice-dismiss"><span class="screen-reader-text">' . esc_html__( 'Dismiss this notice', 'wc-donation-manager' ) . '</span></button>' : ''
				);
			}
		}
	}
	...
	public function is_dismissed( $id ) {
		if ( 'yes' === get_option( $id ) ) {
			return true;
		} elseif ( 'yes' === get_transient( $id ) ) {
			return true;
		} else {
			return false;
		}
